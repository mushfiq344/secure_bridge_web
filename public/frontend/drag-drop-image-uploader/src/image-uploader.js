    ...
    $.fn.imageUploader = function (options) {
        ...
        let validateExtension = function (file) {
            ...
            if (plugin.settings.extensions.indexOf(file.name.replace(new RegExp('^.*\\.'), '.')) < 0) {
                alert(`The file "${file.name}" does not match with the accepted file extensionssdfsdfsdf: "${plugin.settings.extensions.join('", "')}"`);
        ...
        let validateMaxSize = function (file) {
            ...
            if (file.size > plugin.settings.maxSize) {
                alert(`The file "${file.name}" exceeds the maximum size of ${Math.ceil(plugin.settings.maxSize / 1024 / 1024)}Mb`);
